...
4. 用户可使用标注简称来对应相应的工作树路径或分支。
5. 先检查并汇报 `git status --short --branch`、用户选定目标 worktree 内的当前分支、最近提交和相关分支的提交记录。
...
1. 分析目标分支和来源分支的记录，记录最新的共有提交，单独展示短 SHA 和主题信息。
2. 拿到来源分支晚于「共有提交」但不在目标分支上的提交，作为待 pick 的候选，多个提交按祖先到后代的顺序处理。
3. 如果候选提交是 merge commit，必须单独展示其父提交并请求用户明确指定主线父提交；不得自动加入普通 cherry-pick 列表。
4. 输出待 pick 提交的短 SHA 和主题信息，供用户确认。
5. 拼接 `git cherry-pick <sha1> <sha2> <sha3>`，供用户确认后直接执行，此处使用完整提交 SHA。
6. 发生冲突时停止并报告状态。解决后检查冲突标记，运行受影响代码的最窄测试或检查，再执行 `git add` 和 `git cherry-pick --continue`；确认不应应用该提交时，先说明原因，再使用 `git cherry-pick --skip`。
